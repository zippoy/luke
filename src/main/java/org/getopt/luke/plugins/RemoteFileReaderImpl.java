...
public class RemoteFileReaderImpl
    ...
    private static final int rmiPort = 1416;

    private Map<Integer, RandomAccessFileWrapper> openFiles = new HashMap<>();
    ...
    RemoteFileReaderImpl() throws RemoteException {
        super(rmiPort);

        System.out.println(Thread.currentThread().getName() + ": instantiating RemoteFileReaderImpl on rmi port=" + rmiPort);
    }
    ...
    public String[] listAllFiles(String path) throws RemoteException {
        ...
        return dir.list(new FilenameFilter() {
            @Override
            public boolean accept(File dir1, String file) {
                return !new File(dir1, file).isDirectory();
            }
        });
    }
    ...
    static class RandomAccessFileWrapper {
        static final int MAX_IDLE_TIME = 24*3600;
        int id;
        RandomAccessFile file;
        String path;
        long timeStamp;
